...
func (h Handler) environmentsGetVolumes(writer http.ResponseWriter, request *http.Request) {
	...
	systemInfo, err := h.dockerService.GetVolumes(request.Context(), environment.Uri)
	if err != nil {
		log.Error().Err(err).Msg("failed to get volumes")
		return
	}
	...
	if err := json.NewEncoder(writer).Encode(systemInfo); err != nil {
		log.Error().Err(err).Msg("failed to encode volumes")
		writer.WriteHeader(http.StatusInternalServerError)
		writer.Write([]byte(`{"error": "failed to encode volumes"}`))
		return
	}
}
